...
export const UserCount = async () => {
  ...
  return (
    <div className="flex flex-col gap-[20px] p-[20px]">
      ...
      <Text className="font-semibold text-black">{`남은 API 호출 횟수 : ${session?.user?.remainCount.toLocaleString()} 회`}</Text>
      <div className="flex items-center justify-center gap-[10px]">
        <div className="border-gray3 font-pretendard flex flex-1 items-center justify-center rounded-[10px] border p-[10px] font-medium">
          {`카카오 앱 최대 보유량 : ${session?.user?.maxKakaoAppCount.toLocaleString()} 개`}
        </div>
        <div className="border-gray3 font-pretendard flex flex-1 items-center justify-center rounded-[10px] border p-[10px] font-medium">
          {`네이버 앱 최대 보유량 : 준비 중`}
        </div>
